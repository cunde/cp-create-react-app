当前项目部来源于： [Create React App](https://github.com/facebookincubator/create-react-app).
当前项目不讲此脚手架，如果想了解此脚手架点击上文链接

** 此处只讲相关的react语法问题,简单的介绍一下Facebook的这个脚手架吧！ **


## Table of Contents

- [更新到新的发布管理中](#updating-to-new-releases)
- [issue 提交](#sending-feedback)
...
* `create-react-app` 是一个全局的命令行工具，你可以使用它创建一个新项目
* `react-scripts` 是在生产环境中的一个开发依赖

你几乎从不需要更新 `create-react-app` 本身，委托所有的组织到 `react-scripts`.

当你运行 `create-react-app`时,它一直以最新版本的`react-scripts`创建项目，所以你会在自动创建应用时，获得所有最新的产品特征和改进

为了更新一个存在的项目的`react-scripts`的最新版本，[open the changelog](https://github.com/facebookincubator/create-react-app/blob/master/CHANGELOG.md),查看你当前程序包的版本，应用迁移指令更新版本
...
执行完 create-react-app my-app 后，你的项目结构应该如下所示：
...
* `src/index.js` 是JavaScript的入口.

你可以删除或者重命名其他文件。

你可以在`src`中创建子目录。为了更快的rebuilds，只有在`src`中的文件会通过webpack处理<br>
你需要**添加一些JS和CSS文件到`src`中**，否则webpack不能处理他们

只有文件在`public`中，才能被使用到 `public/index.html`中.<br>

无论怎样你都可以创建一些高级目录。<br>
他们不会被发布到生产环境中的，因此我们能使用他们就像文件一样
...
在开发模块中运行应用，会打开[http://localhost:3000](http://localhost:3000)在你的浏览器中

如果你对文件做过修改，页面会重新加载。<br>
你也会看见一些错误提示在页面中

### `npm test`

在监控模块的同时启动单元测试用例。<br>
关于测试章节的更多的信息[running tests](#running-tests)

### `npm run build`

执行`npm run build`命令就会为生产环境构建一个`build`文件。<br>
它能准确的打包React在生产环境中并压缩`build`内的文件

`build`文件是被压缩后并且文件名包含hashes。<br>
你的应用已经准备好被发布了

想了解更多的发布操作[deployment](#deployment)
